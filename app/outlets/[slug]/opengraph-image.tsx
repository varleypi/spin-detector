  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        <div style={{ fontSize: 34, color: '#94a3b8', marginTop: 56 }}>Is</div>
        <div style={{ fontSize: 82, fontWeight: 900, letterSpacing: -2, lineHeight: 1.05 }}>{name}</div>
        <div style={{ fontSize: 34, color: '#94a3b8', marginTop: 8 }}>biased?</div>

        <div style={{ display: 'flex', fontSize: 40, fontWeight: 800, color, marginTop: 40 }}>{leanLabel(mid)}</div>

        {/* Spectrum with marker */}
        <div style={{ position: 'relative', display: 'flex', width: 1020, height: 22, borderRadius: 11, marginTop: 20, background: 'linear-gradient(90deg,#2563eb,#3b82f6,#8b5cf6,#ef4444,#991b1b)' }}>
        ...
        <div style={{ display: 'flex', width: 1020, justifyContent: 'space-between', marginTop: 12, fontSize: 20, color: '#64748b' }}>
          <span>Far Left</span>
          <span>Center</span>
          <span>Far Right</span>
        </div>

        <div style={{ fontSize: 22, color: '#475569', marginTop: 40 }}>Scored daily by Claude &amp; Grok · spindetector.com</div>
      </div>
    ),
    { ...size }
  )
